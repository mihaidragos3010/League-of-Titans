...
const MatchesList = ({ filterLocationId, onFiltersApplied, locations }) => {
    ...
    useEffect(() => {
        if (locations.length === 0) {
            console.warn("No locations available. Ensure LocationList is loaded first.");
            return;
        }
    ...
    const applyLocalFilters = useCallback(() => {
        const filtered = matches.filter((match) => {
            ...
            const matchesLocation =
                locationId.trim() !== ""
                    ? match.match.locationId.toString() === locationId.trim()
                    : true;
            const afterStartDate = startDate ? matchStart >= new Date(startDate) : true;
            const beforeEndDate = endDate ? matchEnd <= new Date(endDate) : true;
        ...
        // Notifică App.js cu datele filtrate
        if (onFiltersApplied) {
